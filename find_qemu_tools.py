...
def find_qemu_img(start_file=__file__, max_up=6):
    ...
    # 1) If frozen by PyInstaller, check _MEIPASS
    try:
        if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
            meipass = Path(sys._MEIPASS)
            for rel in ["", "bin", "qemu", "qemu/bin"]:
                candidate = (meipass / rel / exe_name).resolve()
                if candidate.exists():
                    return candidate
    except Exception:
        pass

    # 2) Check directory of the running executable (useful for bundled apps)
    try:
        exe_dir = Path(sys.executable).resolve().parent
        for rel in ["", "bin", "qemu", "qemu/bin"]:
            candidate = (exe_dir / rel / exe_name).resolve()
            if candidate.exists():
                return candidate
    except Exception:
        pass
    ...
    if sys.platform == "win32":
        pf = [os.environ.get('ProgramFiles'), os.environ.get('ProgramFiles(x86)'), os.environ.get('ProgramW6432')]
        for base in pf:
            if not base:
                continue
            for rel in ["qemu", "qemu\bin", "bin"]:
                candidate = Path(base) / rel / exe_name
                if candidate.exists():
                    return candidate

    # 4) Search upward from the start_file location (development builds)
    start = Path(start_file).resolve()
    start_dir = start.parent
    roots = [start_dir] + list(start_dir.parents[:max_up])
    for root in roots:
        # check common qemu layouts
        for rel in ["qemu", "qemu/bin", "build", "bin"]:
            candidate = (root / rel / exe_name).resolve()
            if candidate.exists():
                return candidate
        # check qemu-*/bin
        qemu_dir = root / "qemu"
        if qemu_dir.exists():
            for sub in sorted(qemu_dir.iterdir(), reverse=True):
                if sub.is_dir() and sub.name.startswith("qemu-"):
                    candidate = (sub / "bin" / exe_name).resolve()
                    if candidate.exists():
                        return candidate
...
def find_icon(name: str, start_file=__file__, max_up: int = 6):
    ...
    """
    # First: if frozen by PyInstaller, check the extraction directory
    try:
        if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
            meipass = Path(sys._MEIPASS)
    ...
    # Next: if running as a frozen single-file executable (not necessarily PyInstaller),
    ...
    # Then: search upward from the source file location (useful during development)
    start = Path(start_file).resolve()
    start_dir = start.parent
    roots = [start_dir] + list(start_dir.parents[:max_up])
    ...
    # Finally: try argv[0] directory as a last resort
    try:
